...
public class RedisIDWorker {
    ...
    private static final long BEGIN_TIMESTEAMP = 1696118400L;
    ...
    public static void main(String[] args) {
        ...
        int dayOfYear = LocalDateTime.now().getYear();
        System.out.println(dayOfYear);
        //初始化LocalDateTime 还是以上线开始使用的时间为准吧
        LocalDateTime dateTime = LocalDateTime.of(
                LocalDateTime.now().getYear(),
                LocalDateTime.now().getDayOfMonth(), 1, 0, 0);
        //转成秒
        long l = dateTime.toEpochSecond(ZoneOffset.UTC);
